...
function obtener_cliente_por_id($id_fiscal) {
    $pdo = conectarBD();
    $stmt = $pdo->prepare("SELECT id_cliente, nombre, apellidos, telefono, email, id_fiscal,domicilio,poblacion, codigo_postal, provincia, direccion_envio, poblacion_envio, codigo_postal_envio, sitio_web, activo FROM contactos WHERE id_contacto = :id_contacto");
    $stmt->execute([':id_fiscal' => $id_fiscal]);
    return $stmt->fetch(PDO::FETCH_ASSOC);
}

//Guardar un nuevo contacto (insertar)
function guardar_contacto($nombre, $apellidos, $telefono, $email, $id_fiscal, $domicilio, $poblacion, $codigo_postal, $provincia, $direccion_envio, $poblacion_envio, $codigo_postal_envio, $sitio_web) {
...
function actualizar_cliente($nombre, $apellidos, $telefono, $email, $id_fiscal, $domicilio, $poblacion, $codigo_postal, $provincia, $direccion_envio, $poblacion_envio, $codigo_postal_envio, $sitio_web) {
    ...
    $pdo = conectarBD();
    $stmt = $pdo->prepare("UPDATE contactos SET nombre = :nombre, apellidos = :apellidos, telefono = :telefono, email = :email, domicilio = :domicilio, poblacion = :poblacion, codigo_postal = :codigo_postal, provincia = :provincia, direccion_envio = :direccion_envio, poblacion_envio = :poblacion_envio, codigo_postal_envio = :codigo_postal_envio, sitio_web = :sitio_web WHERE id_fiscal = :id_fiscal");
    $stmt->execute([
        ':nombre' => $nombre, 
        ':apellidos' => $apellidos, 
        ':telefono' => $telefono, 
...
function eliminar_cliente($id_fiscal) {
    $pdo = conectarBD();
    $stmt = $pdo->prepare("UPDATE contactos SET activo = FALSE WHERE id_fiscal = :id_fiscal");
    $stmt->execute([':id_fiscal' => $id_fiscal]);
}
...
function obtener_clientes_inactivos() {
    $pdo = conectarBD();
    $stmt = $pdo->query("SELECT id_contacto, nombre, apellidos, telefono, email, instagram, tiktok, domicilio, poblacion, provincia, pais FROM contactos WHERE activo = FALSE");
